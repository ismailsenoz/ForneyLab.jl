...
@testset "MSwitchingGaussianGGD" begin
    ...
    dist_A = [randn(1) for i=1:2]
    dist_Q = [[1.0] for i=1:2]
    xi, W = unsafeWeightedMeanPrecision(ruleMSwitchingGaussianGGD(Message(Univariate,GaussianMeanVariance,m=zeros(1),v=diageye(1)),Message(Univariate,GaussianMeanVariance,m=zeros(1),v=diageye(1)),cat,dist_A,dist_Q))
    @test ruleMSwitchingGaussianGGD(Message(Univariate,GaussianMeanVariance,m=zeros(1),v=diageye(1)),Message(Univariate,GaussianMeanVariance,m=zeros(1),v=diageye(1)),cat,dist_A,dist_Q) == ProbabilityDistribution(Multivariate,GaussianWeightedMeanPrecision,xi=xi,w=W)
end
#
#
